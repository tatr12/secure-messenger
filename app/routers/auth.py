...
from app.services import generate_verification_token, send_verification_email, store_verification_token, verify_token
...
async def register(data: RegisterSchema, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return JSONResponse(
            status_code=201, 
            content={"status": "success", "message": "Registration successful. Check your email for verification link."}
        )
    except Exception as e:
...
@router.get("/verify")
async def verify_email(token: str = Query(..., description="Verification token"), db: AsyncSession = Depends(get_db)):
    # Verify token
    user_data = await verify_token(token)
    if not user_data:
        return JSONResponse(status_code=400, content={"error": "Invalid or expired verification token"})
...
async def login(data: dict, db: AsyncSession = Depends(get_db)):
    ...
    if not user.is_verified:
        return JSONResponse(status_code=403, content={"error": "Email not verified. Please check your inbox."})
...
@router.get("/search")
async def search_users(q: str = "", exclude: str = "", db: AsyncSession = Depends(get_db)):
    if not q: return []
    repo = UserRepository(db)
...
async def update_profile(username: str, data: UpdateProfileSchema, db: AsyncSession = Depends(get_db)):
    repo = UserRepository(db)
